...
from decoders import build_decoder, DECODER_REGISTRY  # <-- registry import
...
class Encoder(nn.Module):
    ...
    def forward(self, image: torch.Tensor, watermark=None, alpha=0):
        ...
        return self.post(x) * (1 - alpha) + watermark * alpha
...
class Extractor(nn.Module):
    """
    Thin wrapper around the decoder registry.

    The backbone is selected by ``config.decoder_name`` (default: 'convnext_base').
    Pass ``--decoder_name <name>`` in your bash script to switch backbones.

    Available names (see decoders.py):
    ...
    def __init__(self, config: configs.ModelConfig):
        super().__init__()
        self.config = config
        decoder_name = getattr(config, "decoder_name", "convnext_base")
        print("*" * 100)
        print(
            f"Extractor backbone : {decoder_name}  |  num_encoded_bits : {config.num_encoded_bits}"
        )
        print(f"Available decoders : {', '.join(sorted(DECODER_REGISTRY.keys()))}")
        print("*" * 100)
        self.decoder = build_decoder(decoder_name, config)
